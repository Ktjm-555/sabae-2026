...
export function VenueAreaItem({ area }: VenueAreaItemProps) {
  ...
  const imageBlock = (
    <div
      className={`relative w-full lg:w-[48%] lg:max-w-[687px] lg:shrink-0 ${
  ...
  const textBlock = (
    <div
      className={`flex w-full flex-col lg:w-[52%] lg:max-w-[653px] lg:justify-center ${
        imageFirst ? "order-2" : "order-2 lg:order-1"
      }`}
    >
      <div className="border-b border-[#dcdcdc]">
        <h3
          className={`pb-3 font-display text-[28px] font-bold leading-tight text-primary sm:text-[32px] lg:pb-4 lg:text-[40px] lg:leading-[50px] ${textInset}`}
        >
          {area.title}
        </h3>
      </div>
      <p
        className={`mt-3 font-display text-sm font-bold tracking-wide text-primary-light sm:text-base lg:mt-4 ${textInset}`}
      >
        {area.subtitle}
      </p>
      <p
